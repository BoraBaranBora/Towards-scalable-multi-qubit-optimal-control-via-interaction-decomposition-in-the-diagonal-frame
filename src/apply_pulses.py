...
def apply_pulse(get_U, time_grid, drive, ψ_init, Δ):
    ...
    for i in range(len(time_grid) - 1):
        ...
        U = get_U(drive[i], dt, time_grid[i], Δ)
        ψ_next = U @ states[-1]
...
def plot_population_transfer(states, time_grid, target_indices, initial_label="", save_path=None):
    """Plot population in specified indices over time."""
    time_ns = time_grid.numpy() * 1e9
    pop = torch.stack([torch.abs(s) ** 2 for s in states]).numpy()

    plt.figure(figsize=(8, 6))
    for idx in target_indices:
        plt.plot(time_ns, pop[:, idx], label=f"{initial_label} → {idx}")
    plt.xlabel("Time (ns)")
    plt.ylabel("Population")
    plt.title("State Population Over Time")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    if save_path:
...
result_dir = Path("results") / "pulse_2025-07-24_15-45-02"  # <<--- EDIT THIS
...
for init_idx, target_idx in initial_target_pairs:
    ...
    plot_path = result_dir / f"population_{init_idx}_to_{target_idx}.png"
    plot_population_transfer(states, time_grid, [target_idx], initial_label=init_idx, save_path=plot_path)
